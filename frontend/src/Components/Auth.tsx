...
export default function Auth() {
  ...
  const handleTabChange = (val?: string) => {
    if (val === 'login' || val === 'signup') setActiveTab(val as 'login' | 'signup')
    else setActiveTab(prev => prev === 'login' ? 'signup' : 'login')
  }
  ...
  const Authuser = async () => {
    try {
      ...
      const toastId = toast.loading(activeTab === 'signup' ? 'Signing up...' : 'Signing in...')
      ...
      if (isSignupSuccess || isSigninSuccess) {
        const successMessage = activeTab === 'signup' ? 'Successfully signed up' : 'Successfully logged in'
        ...
        const errMsg = response.data?.message || 'Authentication failed'
        toast.error(errMsg, { id: toastId })
      }
    } catch (e: unknown) {
      // Handle axios-style error objects and generic errors
      let message = 'Something went wrong'
      if (e && typeof e === 'object' && 'response' in e && (e as any).response) {
        message = (e as any).response?.data?.message || (e as any).message || message
    ...
    <div className='min-h-screen bg-black w-full lg:flex'>
      ...
      <div className='flex items-center justify-center py-6'>
        <div className='bg-gray-600 backdrop-blur-sm text-white/60 w-full p-6 text-2xl rounded-lg m-2 lg:m-0'>
          <Tabs defaultValue='signup' value={activeTab} onValueChange={(v) => handleTabChange(v)} className='w-full h-full'>
                      ...
                      <div className='my-3 w-full text-white flex flex-col'>
                        ...
                        <div className='flex'>
                          <input
                            ...
                            className='border-gray-300 h-10 focus:border-none w-full rounded-lg'
                            required
                          />
                          {/* make this a button with type="button" so it doesn't submit the form */}
                          <button type="button" className='text-white/20' onClick={() => setShowPassword(!showPassword)}>
                  ...
                  <CardContent>
                    <div className='my-3 w-full text-white flex flex-col'>
                      ...
                      <input
                        ...
                        className='border-gray-300 h-10 focus:border-gray-3 00  rounded-lg'
                        required
                      />
                    </div>
                    <div className='my-3 w-full text-white flex flex-col'>
